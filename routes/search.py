            ...
            if current_user.is_authenticated:
                ...
                if dtf_start.validate_on_submit():
                    ...
                    try:
                        ...
                        if search_input_handler(form_gender_val) == -1 and search_input_handler(
                                form_is_supporter_val) == -1:
                            schedule_list = ScheduleDatetime.query.join(UserProfile,
                                                                        ScheduleDatetime.user_opening_slot == UserProfile.id).add_columns(
                                ScheduleDatetime.start_at, UserProfile.id, UserProfile.main_language).filter(
                                ScheduleDatetime.start_at >= start_at).filter(
                                UserProfile.main_language == search_input_handler(form_language_val)).all()
                        # if gender is not chosen
                        elif search_input_handler(form_gender_val) == -1:
                            schedule_list = ScheduleDatetime.query.join(UserProfile,
                                                                        ScheduleDatetime.user_opening_slot == UserProfile.id).add_columns(
                                ScheduleDatetime.start_at, UserProfile.id, UserProfile.main_language,
                                UserProfile.is_supporter).filter(ScheduleDatetime.start_at >= start_at).filter(
                                UserProfile.main_language == search_input_handler(form_language_val)).filter(
                                UserProfile.is_supporter == search_input_handler(form_is_supporter_val)).all()
                        # if is_supporter is not chosen 
                        elif search_input_handler(form_is_supporter_val) == -1:
                            schedule_list = ScheduleDatetime.query.join(UserProfile,
                                                                        ScheduleDatetime.user_opening_slot == UserProfile.id).add_columns(
                                ScheduleDatetime.start_at, UserProfile.id, UserProfile.main_language,
                                UserProfile.gender).filter(ScheduleDatetime.start_at >= start_at).filter(
                                UserProfile.main_language == search_input_handler(form_language_val)).filter(
                                UserProfile.gender == search_input_handler(form_gender_val)).all()
                        # if all conditions are chosen 
                        else:
                            schedule_list = ScheduleDatetime.query.join(UserProfile,
                                                                        ScheduleDatetime.user_opening_slot == UserProfile.id).add_columns(
                                ScheduleDatetime.start_at, UserProfile.id, UserProfile.main_language,
                                UserProfile.gender, UserProfile.is_supporter).filter(
                                ScheduleDatetime.start_at >= start_at).filter(
                                UserProfile.main_language == search_input_handler(form_language_val)).filter(
                                UserProfile.gender == search_input_handler(form_gender_val)).filter(
                                UserProfile.is_supporter == search_input_handler(form_is_supporter_val)).all()
                ...
                return render_template('search.html', year=YEAR,
                                       form_start=dtf_start, form_search=form, message="Validation error")
        except TemplateNotFound:
            return abort(404)
    else:
        try:
            ...
            return render_template('search.html', year=YEAR, form_start=dtf_start, form_search=form)
        except TemplateNotFound:
            return abort(404)
